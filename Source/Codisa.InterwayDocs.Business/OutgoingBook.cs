...
namespace Codisa.InterwayDocs.Business
{
    public partial class OutgoingBook
    {
        ...
        private const string QueryList =
            "OutgoingRegisters.RegisterId, RegisterDate, DocumentType, DocumentReference, DocumentEntity, DocumentDept, DocumentClass, DocumentDate, Subject, SendDate, RecipientName, RecipientTown, Notes, ArchiveLocation";

        private string GetOutgoingBookInlineQuery(OutgoingBookCriteriaGet crit)
        {
        ...
        private string GetSelectPart(OutgoingBookCriteriaGet crit)
        {
            var query = "SELECT " + QueryList + Environment.NewLine + "FROM OutgoingRegisters";
            if (!string.IsNullOrEmpty(crit.FullText))
                query += ", OutgoingView";
        ...
        private string GetWherePart(string query, OutgoingBookCriteriaGet crit, ref ClauseStatus clauseStatus)
        {
            QueryBuilder.AddClause(ref query, ref clauseStatus);

            if (!string.IsNullOrEmpty(crit.FullText))
            {
                query += "OutgoingView.RegisterId = OutgoingRegisters.RegisterId";
            ...
            if (crit.SelectedDateTypeName == "AllDates")
            {
                if (!crit.CriteriaStartDate.IsEmpty && !crit.CriteriaEndDate.IsEmpty)
                {
                    ...
                    for (var dateTypeItem = 0; dateTypeItem < crit.DateTypeList.Count - 1; dateTypeItem++)
                    {
                        ...
                        query += "(";
                        query += QueryBuilder.NormalizeToDate("OutgoingRegisters.", dateTypeName) +
                                 " >= @CriteriaStartDate";
                        clauseStatus = ClauseStatus.ClauseAdded;
        ...
        {
            query += Environment.NewLine + "ORDER BY OutgoingRegisters.RegisterId DESC" + Environment.NewLine;
            return query;
        }

        private void BuildWhereClause(OutgoingBookCriteriaGet crit, string dateTypeName, ref string query,
            ref ClauseStatus clauseStatus)
        {
            if (!crit.CriteriaStartDate.IsEmpty)
            {
                QueryBuilder.AddClause(ref query, ref clauseStatus);
                query += QueryBuilder.NormalizeToDate("OutgoingRegisters.", dateTypeName) + " >= @CriteriaStartDate";
                clauseStatus = ClauseStatus.ClauseAdded;
            }
            ...
            {
                QueryBuilder.AddClause(ref query, ref clauseStatus);
                query += QueryBuilder.NormalizeToDate("OutgoingRegisters.", dateTypeName) + " <= @CriteriaEndDate";
                clauseStatus = ClauseStatus.ClauseAdded;
            }
        }
        ...
        partial void OnFetchPre(DataPortalHookArgs args)
        {
            ...
            var crit = args.CriteriaArg as OutgoingBookCriteriaGet;
